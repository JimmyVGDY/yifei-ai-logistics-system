...
临时 SQL schema 必须使用数据库真实物理表名和物理列名，不能使用前端展示字段或 VO 别名。例如费用表应使用 `payable_fee/actual_fee`，车辆表应使用 `load_capacity_kg/volume_capacity_cubic`，运单表没有 `order_no` 时需要通过 `order_id` 关联 `logistics_order`。
