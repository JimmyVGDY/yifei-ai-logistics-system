...
| Python | 3.12+ | 3.11+ | AI 服务必需 | Python AI 服务运行环境，路径 `F:\Development\Tools\Python312\`。 |
| uv | 0.11+ | 0.10+ | AI 服务必需 | Python 包管理器（替代 pip），`pip install uv` 安装。 |
...
| Ollama | 当前稳定版（>=0.1.0） | 系统服务，端口 `11434` | 11434 | AI 记忆和 RAG 建议启动 | Embedding 模型运行时。 |
| Jaeger | 1.72.0 | `F:\Development\Tools\jaeger\jaeger-all-in-one.exe` | 4318 / 16686 | AI 链路追踪建议启动 | OpenTelemetry 分布式追踪；OTLP HTTP 接收 :4318，UI :16686。不启动不影响 AI 问答。 |
...
```text
MySQL + Redis + RabbitMQ + Elasticsearch + Qdrant + Ollama + Jaeger + Nacos + Sentinel + XXL-Job + Java后端 + Python AI服务 + 前端
```

> **AI 模块正在从 Java (Spring AI) 迁移到 Python (FastAPI)**。当前迁移处于第一阶段：Python 服务已搭建、Java 内部端点已就绪、`app.ai.python.enabled` 开关控制。详见 [ADR 0001 — Java+Python 混合架构](adr/0001-java-python-hybrid-architecture.md) 和 [Python AI 服务开发指南](python-ai-service.md)。
...
- [Spring AI 接入说明](spring-ai.md)（迁移前的 Java 实现，迁移完成后将归档）
