...
| Qdrant | 1.18.2 | `F:\Development\Middleware\qdrant\qdrant-1.18.2` | 6333 | AI 长期记忆建议启动 | 账号级长期记忆向量召回；数据目录 `F:\Development\Middleware\qdrant\data`。 |
...
| Qdrant | 1.18.2 | 本地二进制服务 | AI 助手账号级长期记忆向量召回。 |
...
AI 长期记忆默认尝试连接本地 Qdrant，连接失败会降级，不阻断应用启动和物流业务。
...
- Qdrant 只作为 AI 长期记忆向量召回服务，MySQL 中的 `ai_user_memory` 才是可审计、可删除的长期记忆真值。
