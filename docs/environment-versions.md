...
Qdrant 本地脚本位于仓库 `ops/start-qdrant.bat`、`ops/status-qdrant.bat`、`ops/stop-qdrant.bat`。健康检查地址为 `http://127.0.0.1:6333/readyz`。Qdrant 不可用时只影响 AI 长期记忆向量召回和 RAG 文档语义检索，不影响物流主业务和 AI 只读业务查询。当前默认 embedding 模型为 Ollama `bge-m3`，集合维度应为 1024；长期记忆集合默认为 `logistics_ai_user_memory`，RAG 文档集合默认为 `logistics_docs`。如果本地存在旧版 128 维集合，需要重建集合或改用新的集合名。
...
| Qdrant | 1.18.2 | 本地二进制服务 | AI 助手账号级长期记忆向量召回和 RAG 文档向量检索。 |
