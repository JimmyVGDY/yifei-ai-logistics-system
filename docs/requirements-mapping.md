...
| 操作日志 | 新增操作日志拦截器，物流写操作写入 `sys_operation_log`，含脱敏后的变更摘要、安全请求参数、清洗后的 `error_message` 字段支持排障；前端列表页 userId/IP 脱敏展示 |
| 中间件业务链路 | 创建订单后写 Redis 缓存、BloomFilter、ES 索引，发送 RabbitMQ 事件并初始化物流轨迹 |
| AI 助手 | 已落地只读 AI 助手（系统问答+业务查询+日志排障+受控 SQL+长期记忆），Java+Python 混合架构。详见 [AI 助手设计](ai-assistant-design.md) |
