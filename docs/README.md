...
- 使用或继续开发 AI 助手前先阅读 [spring-ai.md](spring-ai.md)，再阅读 [ai-assistant-design.md](ai-assistant-design.md)；涉及 AI 生成临时查询时还必须阅读 [mybatis.md](mybatis.md) 中的例外边界，涉及历史会话时确认 `ai_conversation` / `ai_conversation_message` 增量脚本，涉及长期记忆时还要确认 Qdrant 版本和 `ai_user_memory` 相关增量脚本。
