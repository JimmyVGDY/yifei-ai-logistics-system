...
const moduleMetas = {
  ...
  operationLogs: { title: '操作日志', description: '记录关键接口和业务写操作', editable: false, columns: columns('operation_id:操作ID,trace_id:Trace ID,user_code:用户编号,user_id:用户主键,username:操作人,role_code:角色编号,operation:操作内容,target_id:对象ID,client_ip:客户端IP,request_uri:请求地址,request_method:方法,operation_status:状态,cost_ms:耗时ms,request_params:参数摘要,error_message:异常信息,operation_time:操作时间') },
