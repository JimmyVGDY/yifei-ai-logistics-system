...
/**
 * AI 助手主服务 —— 编排知识检索、日志排障、模型调用和会话缓存。
 */
@Slf4j
@Service
public class AiAssistantService {
    ...
    public AiChatResponse chat(AiChatRequest request) {
        ...
        List<AiCitation> citations = knowledgeService.search(safeMessage);
        List<AiToolCall> toolCalls = new ArrayList<>();

        String context = contextText(citations);
        if (looksLikeLogQuestion(safeMessage)) {
            ...
            context += "\n日志排障摘要：" + analysis.summary();
            toolCalls.add(new AiToolCall("operationLogAnalyze", "ai:log:analyze", "operationLogs", analysis.summary()));
        }

        String systemPrompt = """
                你是物流管理系统的 AI 助手。只能做只读问答、系统使用说明和日志排障。
                不允许承诺已经新增、修改或删除数据。涉及写操作时，只能给出人工操作路径和注意事项。
        ...
        String answer = modelAnswer.orElseGet(() -> fallbackAnswer(safeMessage, citations, toolCalls));
    ...
    private String fallbackAnswer(String message, List<AiCitation> citations, List<AiToolCall> toolCalls) {
        StringBuilder builder = new StringBuilder();
        if (!modelGateway.configured()) {
            builder.append("当前未配置模型 API Key，已返回本地只读检索结果。");
        } else {
            builder.append("模型调用暂不可用，已返回本地只读检索结果。");
        }
        if (!toolCalls.isEmpty()) {
            builder.append("\n\n已执行工具：");
            for (AiToolCall toolCall : toolCalls) {
                builder.append("\n- ").append(toolCall.toolName()).append("：").append(toolCall.result());
            }
        }
        ...
        if (citations.isEmpty() && toolCalls.isEmpty()) {
            builder.append("\n\n没有找到足够的本地上下文。你可以补充 traceId、operationId、loginSessionId、订单号或具体页面名称。");
        }
    ...
    private boolean looksLikeLogQuestion(String message) {
        ...
        return lower.contains("traceid") || lower.contains("operationid") || lower.contains("loginsessionid") || lower.contains("日志") || lower.contains("报错");
    }
